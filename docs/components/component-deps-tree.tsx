...
export function ComponentDepsTree({
  ...
  if (!item) {
    return (
      <div className="my-4 rounded-xl border border-destructive/50 bg-destructive/10 p-4 text-sm text-destructive">
  ...
  if (files.length === 0) {
    return (
      <div className="my-4 rounded-xl border border-muted p-4 text-sm text-muted-foreground">
  ...
  return (
    <div className={`not-prose my-4 ${className ?? ""}`.trim()}>
      <Files>{renderTreeNodes(tree, defaultOpen)}</Files>
    </div>
  )
}
